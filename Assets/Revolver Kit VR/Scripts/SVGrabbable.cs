...
public class SVGrabbable : MonoBehaviour {
    ...
    void Start () {
        ...
        /*
        if( this.input.m_controller == OVRInput.Controller.RTouch )
        {
            TrySetActiveController(SVControllerType.SVController_Right);
        }
        else if (this.input.m_controller == OVRInput.Controller.LTouch)
        {
            TrySetActiveController(SVControllerType.SVController_Left);
        }
        */
    ...
    private void UngrabbedUpdate() {
        ...
        if (grabDistance > distanceToLeftHand ||
			...
			if (distanceToLeftHand < distanceToRightHand) {
                if( SVControllerManager.nearestGrabbableToLeftController == null ||
                    SVControllerManager.nearestGrabbableToLeftController == this)
					firstController = SVControllerType.SVController_Left;

                if (SVControllerManager.nearestGrabbableToRightController == null ||
