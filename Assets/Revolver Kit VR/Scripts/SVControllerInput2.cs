...
public class SVControllerInput2 : MonoBehaviour
{
    ...
    void Start()
    {
		...
		Assert.IsNotNull (controllerManager, "SVControllerInput2 (with SteamVR) Needs a SteamVR_ControllerManager in the scene to function correctly.");
#elif USES_OPEN_VR
        controllerManager = Object.FindObjectOfType<OVRManager>();
        Assert.IsNotNull(controllerManager, "SVControllerInput2 (with Open VR) Needs a OVRManager in the scene to function correctly.");
        ...
        {
            SetActiveController(SVControllerType.SVController_Right);
        }
        else if( m_controller == OVRInput.Controller.LTouch )
        {
            SetActiveController(SVControllerType.SVController_Left);
        }
        else
        {
            SetActiveController(SVControllerType.SVController_None);
        }
#else
